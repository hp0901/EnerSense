...
//   if (!card) {
//     console.log("Card is not available yet:", card);
//     return <div className="text-white">Loading card...</div>;
//   }

// const style =
//   cardStyles[card?.cardType] || cardStyles.Bronze;
...
//         >
//           {(card?.cardType || "Bronze").toUpperCase()} CARD        </div>
//       </div>
...
const UserCard = ({ card }) => {
  ...
  const cardType = card?.cardType || "Bronze";
      ...
      <div className="p-5 md:p-8 flex flex-col md:flex-row gap-6 md:gap-10 items-center md:items-start">
        ...
        <div className="flex-1 w-80 text-center">
          ...
          <div className="grid grid-cols-2 gap-y-3 gap-x-6 text-[11px] md:text-sm uppercase font-bold text-slate-700">
            ...
            <InfoItem label="User UID" value={card?.userUID} />
            <InfoItem label="Board UID" value={card?.boardUID} />
            <InfoItem label="Phone" value={card?.phone ? maskPhone(card.phone) : "—"} />
            <InfoItem label="Devices" value={`${card?.totalDevices ?? 0} UNITS`} />
